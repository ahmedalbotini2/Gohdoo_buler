...
class OverlayManager(private val context: Context) {
    ...
    // ✅ جديد: هل النظام يدعم فعلياً "البلور المتقاطع بين النوافذ" في اللحظة
    // الحالية؟ هذه القيمة يتحكم بها نظام أندرويد نفسه (تُعطَّل تلقائياً مثلاً
    // أثناء تفعيل موفر شحن البطارية، أو قد لا تكون مدعومة إطلاقاً على بعض
    // الأجهزة/واجهات الشركات المصنّعة المخصصة). بدون هذا التحقق، تفعيل
    // FLAG_BLUR_BEHIND لا يُنتج أي بلور مرئي فعلي مهما كانت قيمة
    // blurBehindRadius، ويظهر فقط تظليل اللون كخلفية — وهو بالضبط ما يظهر
    // للمستخدم كـ"ظل أسود خفيف" بدل بلور حقيقي. نتحقق من القيمة عند الإنشاء
    // ونستمع لأي تغيّر لاحق فيها لنعيد بناء الحجب تلقائياً وفقها.
    ...
    private fun buildFullScreenOverlay(container: FrameLayout) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.S) {
            if (crossWindowBlurEnabled) {
                // ✅ بلور حقيقي مؤكَّد الدعم فعلياً من النظام في هذه اللحظة
                container.setBackgroundColor(0x55000000.toInt())
                applyBlurFlagToWindow()
            } else {
                // لا يوجد دعم فعلي للبلور حالياً على هذا الجهاز/هذه اللحظة
                // (موفر شحن البطارية مفعّل، أو الواجهة المخصصة للجهاز لا
                // تدعمه) → لا فائدة من تفعيل الفلاغ، فنعتمد على نفس التظليل
                // كبديل احتياطي (كان هذا هو نفس السلوك الظاهر سابقاً أصلاً)
    ...
    private fun applyBlurToCurrentOverlay() {
        if (Build.VERSION.SDK_INT < Build.VERSION_CODES.S) return
        if (!crossWindowBlurEnabled) return // لا فائدة من التحديث، البلور غير مدعوم فعلياً حالياً
        val regions = currentRegions
        // البلور الكامل (FLAG_BLUR_BEHIND) يُحدَّث فقط في حالة التغطية الكاملة
        if (regions == null || regions.isEmpty()) {
            val params = overlayParams ?: return
